...
Not listed here by hand. The hand-kept table that used to sit here went stale: it named
`arms`, which is not installed on this machine, and a launcher button trusted it.

The live list is **`OS/SKILLS.html`**, regenerated by `build_home.py` from
`~/.claude/skills/` on every rebuild. Each skill opens to its full `SKILL.md`, and its
`README.md` when it has one. HOME.html links every skill into it.

Account skills sync to every session including Cowork, but they are not on disk, so the
page cannot list them. Skills installed only in the CLI under `~/.claude/skills/` stay
on this machine.
