// Today: the front page. On the left, what needs attention across Andrew's projects, read live
// from disk (/api/projects, the same data as Projects.tsx): repos with uncommitted work, each
// course's projects, and Blueberry's newest STATUS.md entry. On the right, the things done here
// most: quick capture, pinned files and the skills that run on this computer (/api/tasks).
// No hero, no slogan, no stat cards and no parallax (Andrew's decision of 2026-09-14).
import { useEffect, useLayoutEffect, useRef, useState } from 'react';
import { ArrowUpRight, BookOpen, GitBranch, GraduationCap, Loader2, Pencil, Pin, Plus, TerminalSquare, Zap } from 'lucide-react';
import type { Doc } from './types';
import { api } from './lib/api';
import { Markdown } from './Markdown';
import { statusText, type Task } from './RunPanel';
import type { InstalledSkill } from './Skills';
import { ProjectRow, type ProjectsData } from './Projects';

type Props = { data: ProjectsData | null; error: string; installed: InstalledSkill[] | null; pinned: Doc[]; openDoc: (doc: Doc) => void; newDoc: () => void; capture: (text: string) => Promise<boolean> };

const when = (iso: string) => new Date(iso).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' });

export function Today({ data, error, installed, pinned, openDoc, newDoc, capture }: Props) {
  ...
  // Read once per visit, so a run started on a skill's page shows here when you come back.
  useEffect(() => {
    ...
    <div className="today-grid">
      ...
      <div className="today-column">
        <section className="panel capture-panel">
          ...
          <div className="capture-footer"><span>Saved to Files <kbd>Ctrl ↵</kbd></span><button className="button small primary" disabled={!text.trim() || busy} onClick={() => void save()}>{busy ? <Loader2 className="spin" size={14}/> : <Plus size={14}/>}Save note</button></div>
        </section>
        <section className="panel">
          <div className="section-heading"><h2><Pin size={16}/>Pinned</h2><a className="text-button" href="#files">All files<ArrowUpRight size={14}/></a></div>
          {pinned.length ? <div className="pinned-list">{pinned.map(doc => <button className="pinned-item" key={doc.id} onClick={() => openDoc(doc)}><span className={`doc-symbol ${doc.kind}`}>{doc.kind === 'skill' ? <Zap size={20}/> : <BookOpen size={20}/>}</span><span><strong>{doc.name}</strong><small>{doc.tags[0] || doc.kind}</small></span><ArrowUpRight size={16}/></button>)}</div> : <p className="today-note">Pin a note or file on Files to keep it here.</p>}
        </section>
        <section className="panel">
          ...
          {tasks === null ? <p className="today-note"><Loader2 className="spin" size={15}/>Reading runs…</p> : tasksError ? <p className="today-note error-text" role="alert">{tasksError}</p> : tasks.length ? tasks.map(task => <a className="today-run" key={task.id} href={'#skills/' + (installed?.find(skill => skill.task === task.id)?.slug || task.id)}>
