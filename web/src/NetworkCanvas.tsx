...
declare global { interface Window { __network?: { ready: boolean; count: number; screenOf: (path: string) => { x: number; y: number } | null; camera: () => Camera; renders: () => number; selected: () => string; colorOf: (path: string) => string; colors: Record<string, string> } } }
...
export function NetworkCanvas({ model, scene, selected, hovered, onSelect, onHover, focus, sceneKey }: Props) {
  ...
  const state = useRef({ cam: { tx: 0, ty: 0, k: 1 } as Camera, fitK: 0.05, cache: null as null | (Camera & { off: HTMLCanvasElement; w: number; h: number }), grid: new Map<string, number[]>(), w: 0, h: 0, dpr: 1, frame: 0, timer: 0, renders: 0, drag: null as null | { x: number; y: number; node: number; moved: boolean }, hover: -1 });
  ...
  function renderCache() {
    ...
    const off = s.cache?.off && s.cache.w === w && s.cache.h === h ? s.cache.off : document.createElement('canvas');
    off.width = Math.round(w * 2 * dpr); off.height = Math.round(h * 2 * dpr);
    ...
    o.setTransform(dpr, 0, 0, dpr, 0, 0); o.textAlign = 'center'; o.textBaseline = 'top'; o.lineJoin = 'round'; o.lineWidth = 3.5; o.strokeStyle = 'rgba(13,15,18,0.95)';
    ...
    for (const [, p, size, weight] of want) {
      ...
      const node = model.nodes[scene.ids[p]]; o.font = `${weight}${size}px "DM Sans Variable", sans-serif`;
      const width = o.measureText(node.name).width; const sx = scene.x[p] * cam.k + ox, sy = scene.y[p] * cam.k + oy + (node.kind === 'dept' ? -dotRadius(p) - 4 - size : dotRadius(p) + 3);
      if (!free(sx - width / 2 - 3, sy - 1, width + 6, size + 4)) continue;
      placed.push([sx - width / 2 - 3, sy - 1, width + 6, size + 4]);
      o.fillStyle = node.kind === 'dept' ? '#f2f1ef' : node.layer ? colorOf(node) : '#c9cad2';
      o.strokeText(node.name, sx, sy); o.fillText(node.name, sx, sy);
    }
    s.cache = { off, w, h, tx: cam.tx, ty: cam.ty, k: cam.k }; s.renders++;
  }
  ...
  function draw() {
    ...
    if (focusNode >= 0 && scene.at[focusNode] >= 0) {
      ...
      // Everything outside the neighbourhood drops to near-invisible, so the lit names are the only names.
      ctx.fillStyle = 'rgba(13,15,18,0.9)'; ctx.fillRect(0, 0, w, h);
      ...
      for (const q of near) { if (scene.at[q] < 0) continue; const isLink = model.linksOut[focusNode].includes(q) || model.linksIn[focusNode].includes(q); if (isLink) { ctx.moveTo(sx(p), sy(p)); ctx.lineTo(sx(scene.at[q]), sy(scene.at[q])); } }
      ctx.stroke();
      ctx.font = '11px "DM Sans Variable", sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.lineWidth = 3; ctx.lineJoin = 'round';
      // A label is drawn only where no earlier label sits, so a folder's packed contents never smear into one another.
      const r = Math.max(4, dotRadius(p)) + 1.5; ctx.font = '600 12.5px "DM Sans Variable", sans-serif'; const width = ctx.measureText(node.name).width; const lx = sx(p), ly = sy(p) + r + 5;
      const placed: [number, number, number, number][] = [[lx - width / 2 - 7, ly - 3, width + 14, 21]];
      const free = (x: number, y: number, w2: number, h2: number) => placed.every(([px, py, pw, ph]) => x + w2 < px || px + pw < x || y + h2 < py || py + ph < y);
      ctx.font = '11px "DM Sans Variable", sans-serif'; let labels = 0;
      for (const q of near) { const pq = scene.at[q]; if (pq < 0) continue; const rq = Math.max(2.5, dotRadius(pq)); ctx.fillStyle = colorOf(model.nodes[q]); ctx.beginPath(); ctx.arc(sx(pq), sy(pq), rq, 0, 6.2832); ctx.fill();
        if (labels >= 90) continue; const name = model.nodes[q].name; const tw = ctx.measureText(name).width; const tx = sx(pq) - tw / 2, ty = sy(pq) + rq + 3;
        if (!free(tx, ty, tw, 13)) continue; placed.push([tx, ty, tw, 13]); labels++;
        ctx.strokeStyle = 'rgba(13,15,18,0.92)'; ctx.fillStyle = '#d9d9df'; ctx.strokeText(name, sx(pq), ty); ctx.fillText(name, sx(pq), ty); }
      ...
      ctx.fillStyle = 'rgba(13,15,18,0.94)'; ctx.beginPath(); ctx.roundRect(lx - width / 2 - 7, ly - 3, width + 14, 21, 6); ctx.fill();
      ctx.strokeStyle = colorOf(node); ctx.lineWidth = 1; ctx.globalAlpha = 0.45; ctx.stroke(); ctx.globalAlpha = 1;
      ctx.fillStyle = '#f2f1ef'; ctx.fillText(node.name, lx, ly);
    }
  ...
  function fitScene() {
    ...
    const k = Math.min(3, Math.max(0.05, Math.min((s.w - 90) / (maxX - minX + 1), (s.h - 90) / (maxY - minY + 1))));
