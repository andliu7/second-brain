...
This workspace has a second brain at `{brain}`.

**Check the catalogue first, open files second.** `{brain}/MEMORIES.md` is one line per
memory: date, kind, link and a one-sentence description, for every saved memory and every
standing note in `OS/memory/`. It is small, so read it whole. It is generated by `catalogue.py`
from the memory files themselves and rewritten on every save, so it cannot drift; never edit it
by hand, and never treat a memory as missing because it is absent from this file without
running `python "{brain}/catalogue.py" --check` first.

**Decisions, research and meetings live in the wiki**, `{brain}/wiki/`: agent-written pages
with `index.md` as the catalogue, `log.md` as the timeline and `SCHEMA.md` as the rules for
ingesting, querying and linting it. Read `index.md` before opening pages. Write findings there
rather than answering from scratch twice, and index each page with `idx.py --file` as you go.
The wiki never covers code or file locations: `q.py` answers those from byte offsets it checks
against disk, and a wiki page is a paraphrase that is allowed to disagree.

Before using Glob/Grep/Read to answer a
question about my files, notes, decisions or preferences, run:

    python "{brain}/q.py" "<the question>"

It prints an evidence bundle in well under a second (about 0.6s measured, no model call): `BRAIN <path>#<section> [flags score/margin]`, an
